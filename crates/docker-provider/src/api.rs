...
async fn reconcile(workers: &mut BTreeMap<String, String>) -> anyhow::Result<Vec<Worker>> {
    let states = docker::states(workers.values().map(String::as_str)).await?;
    workers.retain(|_, c| states.contains_key(c));
    Ok(workers
        .iter()
        .map(|(w, c)| Worker { worker_id: w.clone(), container_id: c.clone(), status: states[c].clone() })
        .collect())
}
...
async fn stop_worker(State(state): State<AppState>, Path(id): Path<String>) -> Result<StatusCode, ApiError> {
    let mut workers = state.workers.lock().await;
    let Some(container_id) = workers.get(&id) else {
        return Err(ApiError::NotFound(format!("no worker {id}")));
    };
    docker::remove(container_id).await?;
    workers.remove(&id);
...
async fn status(State(state): State<AppState>) -> Result<Json<Status>, ApiError> {
    ...
    Ok(Json(Status {
        capacity: state.config.provider.max_workers,
        in_use: in_use(&workers),
        image: state.config.docker.image.clone(),
        workers,
    }))
}
